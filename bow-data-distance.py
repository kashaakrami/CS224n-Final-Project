...
print("Method: maximize data distance (goal-driven learning)")
...
for i in range(active_learning_steps):
    ...
    if len(current_train_indices) == 0:
        ...
        current_data = X_train[list(current_train_indices)]
        center = current_data.mean(axis=0)
    
    distances = np.linalg.norm(X_train - center, axis=1)

    indices = np.argsort(np.abs(distances.flatten()))[::-1]
